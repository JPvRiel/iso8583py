...
# Exception used to indicate the version is incorrect or unsupported
class InvalidVersion(Exception):
  """Exeption occurs when the version is not known or supported by this libary. This occurs if an invalid
          paramater is passed when setting the version, or if there is an unsupported or unkown specification.
  """
...
# Exception used to indicate that the value that setting in the bit is
# large than the iso limit to that bit!
...
class InvalidValueType(Exception):
  """Exeption that indicate that a value that you try to insert is not in the specification.
